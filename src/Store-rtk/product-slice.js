...
export const fetchProducts = () => async(dispatch) => {
  ...
  response.data.forEach(product => { dispatch(create(product)) })
}
...
export const adjustProduct = (item, method) => async(dispatch) => {
  ...
  const response = await axios.put(`${base}/${item._id}`, { inventory: updInventory })
...
export const { create, adjust } = productSlice.actions;
