...
export function updateProduct(item, action) {
  ...
  return async function reduceQuantity(dispatch, getState) {
    ...
    if (action === "add") {
      ...
      if (updInventory < 0) { return }
      const response = await axios.put(`${base}/${itemId}`, { inventory: updInventory })
      dispatch({ type: 'product/added', payload: { updItem: response.data, method: action }})
    } else {
      const updInventory = item.inventory + 1
      const response = await axios.put(`${base}/${itemId}`, { "inventory": updInventory })
      if (action === "subtract") {
        dispatch({ type: 'product/subtracted', payload: { updItem: response.data, method: action }})
      } else {
        dispatch({ type: 'product/deleted', payload: { updItem: response.data, method: action }})
      }
    }
  }
}
